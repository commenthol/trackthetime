...
Report.prototype = {
  ...
  _typePrj: function (from, to, type, selectPrjs) {
    let titleDefault
    let isCummulate = false
    const obj = { sum: 0 }
    const cache = {}
    const hasSelectedPrjs = !!selectPrjs
    const selected = Report.selectProjects(selectPrjs)

    let items = this._loop(from, to, type)

    if (type === WEEK) {
      const itemsDay = this._loop(from, to, DAY)
      if (itemsDay.length < 7) {
        // report partial week
        titleDefault = `CW${items[0]}p`
        items = itemsDay
        type = DAY
        isCummulate = true
      }
    ...
    items.forEach(function (item) {
      ...
      const title = titleDefault || (type === WEEK ? 'CW' + item : item)

      for (const p in prjs) {
        let cummulatedSecs = 0
        ...
        if (secs && selected[p] !== false) {
          ...
          if (isCummulate) {
            const cached = cache[title][p] || 0
            cummulatedSecs = secs + cached
            cache[title][p] = cummulatedSecs
          }
          obj[title][p] = this._toHour(cummulatedSecs || secs, 1)
          if (p !== PAUSE) {
            obj.sum += secs
          }
        }
