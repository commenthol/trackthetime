 ...
 * constants
 */
const { DATE, TIME, DATE_REX, TIME_REX } = require('./consts')
...
function Task (options) {
  ...
  Object.defineProperty(this, 'month', {
    get: function () {
      const month = this.date.match(/^(\d{4}-\d{2}).*$/)
...
Task.prototype = {
   ...
   */
  toString: function () {
    const arr = [this.date, this.time, this.project, this.description]
    let str = arr.join('\t')
    if (this.deleted) {
      str = '#' + str + '#DELETED#'
    }
